...
class DirectoryFileSystemHandler(FileSystemEventHandler):
    ...
    def _event_handler(self, path):
        ...
        if ext not in (".tmp", ".part", ".crdownload") and ext[:2] not in (".~"):
            ...
            if filesize > MAX_UPLOAD_FILE_SIZE:   # was 10000000
                logging.warning(f"Filesize = {filesize}. Maybe too big for Joplin, skipping upload")
                return False
            else:
                _priority = 10
            ...
            logging.info("Detected temp file. Temp files are ignored.")
    ...
    def on_moved(self, event):
        logging.info(event.event_type + " -- " + event.dest_path)
...
def _rotate_image(filename):
    ...
    logging.info("[INFO] detected orientation: {}".format(
        results["orientation"]))
    logging.info("[INFO] rotate by {} degrees to correct".format(
        results["rotate"]))
...
def extract_text_from_pdf_object(pdf_object, language="deu+eng", auto_rotate=False):
    ...
    if pdf_reader.is_encrypted:
        logging.error('    --NOTICE: This file is encrypted and cannot be read by Joplin OCR\n')
        return None
    text = list()
    preview_file = None
    _pages_num = len(pdf_reader.pages)
    logging.info(f"Pages: {_pages_num}")
    for i, x in enumerate(pdf_reader.pages):
        ...
        if extracted_text_list is not None:
            ...
            logging.info(f"Page {i + 1} of {len(pdf_reader.pages)} processed successfully.")
        else:
            extracted_text = ""
...
def extract_text_from_image_object(image_object, auto_rotate=False, language="eng"):
    ...
    try:
        ...
        logging.error(f"TesseractError {e.message}")
        return None
...
def note_ocr_from_queue(queue): # 
    ...
    if not queue.empty():
        message = queue.get()
        logging.info(f"Uploader received a message: {message}")
        _msg_data = message
        if _msg_data[0] == 'note_ocr':
            ...
            IS_UPLOADING = True
            logging.info(f'OCR running now for note: {_note_id}')
...
def upload(filename): # 
    ...
    if datatype == "text/csv":
        ...
        try:
            ...
            logging.error(f"Invalid or incomplete file - {filename}")
            db_response = db.add_file_info(basefile, ocr_status='fail')
            return -1
        body += "\n-->\n"
        ...
        if os.path.isdir(MOVETO):
            ...
            if os.path.exists(moveto_filename):
                ...
                try:
                    ...
                    logging.error(f"File Locked-unable to move {filename}")
        return 0
    else:
        logging.error("ERROR! NOTE NOT CREATED")
        logging.error("Something went wrong corrupt file or note > max upload file size?")
        return -1
...
def apply_tags(text_to_match, note_id):
    ...
    logging.info(f"Matched {counter} tag(s) for note {note_id}")
    return counter
...
def __ocr_resource(resource, create_preview=True):
    ...
    try:
        ...
        try:
            ...
            logging.error("File: " + str(resource.title))
            return OcrResult(None, success=False)
        except TypeError:
    ...
    if __is_note_ocr_ready(note):
        ...
        if len(result.strip()) > 0:
            ...
            if not DRY_RUN:
                ...
                logging.info("Result: note updated")
            else:
                logging.info("Result: note would have been updated [dry run]")
            return ResultTag.OCR_ADDED
        else:
            logging.warning("Result: note not updated")
            return ResultTag.OCR_FAILED
    else:
        logging.info("Result: skipped")
...
def watcher_helper(path_to_watch=None, _queue=None):
    ...
    #logging.info(f'watcher starting for path: {path_to_watch}')
...
def watcher(path_to_watch=None):
    ...
    logging.info(f"Monitoring directory: {path_to_watch}")
...
def do_something_important():
    ...
    for i in range(64):
        x+=1
    logging.info('counted do_something_important')
    ...
    while _is_running:
        if not IS_UPLOADING:
            if not queue_jobs.empty():
                ...
                logging.info(f"Watcher received a message: {message}")
                _msg_data =message[1]
                if _msg_data[0] == 'upload':
                    _file_path = _msg_data[1]
                    logging.info(f"File path: {_file_path}")
                    IS_UPLOADING = True
                    logging.info(f'Uploading from directory: {_file_path}')
                    upload(_file_path)
                    IS_UPLOADING = False
                    time.sleep(_wait_time_short)
                elif _msg_data[0] == 'res':
                    _res = _msg_data[1]
                    logging.info(f"Resource: {_res}")
                    time.sleep(_wait_time_short)
                ...
                if IS_UPLOADING:
                    ...
                    IS_UPLOADING = True
                    logging.info(f'Starting to OCR notes from TAG. {_queue_len} to go')
...
def __observ_folder_run(observed_folders): # TODO: add support for multiple observed folders
    logging.info(f"Observing folders {observed_folders}.")
    watcher(observed_folders)

def __observ_folder_and_scan_run(observed_folders): # TODO: add support for multiple observed folders
    logging.info(f"Observing folders and scan {observed_folders}.")
    watcher(observed_folders)


def __full_run(tag, exclude_tags):
    logging.info("Starting OCR for tag {}.".format(tag))
    tag_id = Joplin.find_tag_id_by_title(tag)
    if tag_id is None:
        logging.warning("Tag not found or specified")
        return -1
